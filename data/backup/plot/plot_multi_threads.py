...
# Fist subplot
plot_script += base_next_plot.format(title=r"(a) read: mean latency")
plot_script += """
set key noenhanced
...
# Second subplot
plot_script += base_next_plot.format(title=r"(b) write: mean latency")
plot_script += """
unset ylabel
"""
...
plot_script += "\\"
plot_script += """
"data/driver/label_randwrite_data_multi_threads.csv" \
"""
labels = base_line_labels.format(
    entry=f"{baselines[1].id}:{baselines[1].id + len(baselines)}:4",
    offset="-1.5,1",
...
# Third subplot
plot_script += base_font.format(**font_dict)
plot_script += base_next_plot.format(title=r"(c) read: tail latency (p99)")
plot_script += """
set key noenhanced
set ylabel "Latency (us)"
set ytic 15
...
base = baselines[0]
plot_script += base_line_curve_notitle.format(
    entry=f"{base.id}:{next_col(base.id, nr_base, 2)}",
    color=base.color,
    pt=base.point_t,
    title=base.title,
)
...
base = baselines[1]
labels = base_line_labels.format(
    entry=f"{base.id}:{next_col(base.id, nr_base, 2)}:4",
    offset="-1.5,1",
    color="C0",
)
plot_script += "\\" + labels
...
# Forth subplot
plot_script += base_next_plot.format(title=r"(d) write: tail latency (p99)")
plot_script += """
unset ylabel
set ytic 20
set yrange [0:80]
...
# Fifth subplot
plot_script += base_font.format(**font_dict)
plot_script += base_next_plot.format(title=r"(e) read: tail latency (p99.9)")
plot_script += """
set key noenhanced
set xlabel "IOPS (k)"
set ylabel "Latency (us)"
...
# Sixth subplot
plot_script += base_next_plot.format(title=r"(f) write: tail latency (p99.9)")
plot_script += """
unset ylabel
set ytic 20
set yrange [0:80]
