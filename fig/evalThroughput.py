import matplotlib.pyplot as plt
import numpy as np
import tikzplotlib

# Data
batch_sizes = ["2", "4", "8", "16", "32", "64", "128"]

throughput_novaserve = [0, 10.25, 21.97, 28.57, 57.14, 0, 0]
throughput_flexgen = [0, 8.94, 11.85, 25.5, 35.46, 0, 0]

# Bar width and positions
width = 0.35
x = np.arange(len(batch_sizes))

# Colors and hatches
colors = ["#5DADE2", "#FFD700"]  # 深橙 (NovaServe), 深粉 (FlexGen)
colors2 = ["#d55e00", "#A6206A"]  # Deep orange for NovaServe, Deep pink for FlexGen
hatch_style = "/"  # FlexGen hatch style

# Create figure and subplots
fig, axes = plt.subplots(1, 2, figsize=(14, 6))


axes[0].bar(
    x - width / 2, throughput_novaserve, width, label="NovaServe", color=colors2[0]
)
axes[0].bar(
    x + width / 2,
    throughput_flexgen,
    width,
    label="FlexGen",
    color="white",
    edgecolor=colors2[1],
    hatch=hatch_style,
)

axes[0].set_title("(a) Throughput Comparison on Alpaca", fontsize=16)
axes[0].set_xlabel("Batch Size", fontsize=14)
axes[0].set_ylabel("Throughput (tokens/s)", fontsize=14)
axes[0].set_xticks(x)
axes[0].set_xticklabels(batch_sizes, fontsize=12)
axes[0].legend(fontsize=12, loc="best", frameon=False)

# Annotate bars
for i, v in enumerate(throughput_novaserve):
    axes[0].text(i - 0.2, v + 2, f"{v:.2f}", ha="center", fontsize=10)
for i, v in enumerate(throughput_flexgen):
    axes[0].text(i + 0.2, v + 2, f"{v:.2f}", ha="center", fontsize=10)



# Right plot: Throughput
axes[1].bar(
    x - width / 2, throughput_novaserve, width, label="NovaServe", color=colors2[0]
)
axes[1].bar(
    x + width / 2,
    throughput_flexgen,
    width,
    label="FlexGen",
    color="white",
    edgecolor=colors2[1],
    hatch=hatch_style,
)

axes[1].set_title("(b) Throughput Comparison on LongBench", fontsize=16)
axes[1].set_xlabel("Batch Size", fontsize=14)
# axes[1].set_ylabel("Throughput (tokens/s)", fontsize=14)
axes[1].set_xticks(x)
axes[1].set_xticklabels(batch_sizes, fontsize=12)
axes[1].legend(fontsize=12, loc="best", frameon=False)

# Annotate bars
for i, v in enumerate(throughput_novaserve):
    axes[1].text(i - 0.2, v + 2, f"{v:.2f}", ha="center", fontsize=10)
for i, v in enumerate(throughput_flexgen):
    axes[1].text(i + 0.2, v + 2, f"{v:.2f}", ha="center", fontsize=10)


# Adjust layout
plt.tight_layout()
output_path = "figures/evalThroughput.tex"
tikzplotlib.save(output_path)

